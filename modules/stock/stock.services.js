...
export const moveStock = async (
  ...
  // ── Determine direction ─────────────────────────────────────
  const inMovements  = ["purchase_in", "sales_return", "transfer_in",  "adjustment_in",  "opening_stock"];
  const outMovements = ["sales_out",   "purchase_return", "transfer_out", "adjustment_out"];
...
export const getStock = async ({ companyId, warehouseId, itemId, financialYearId }) => {
...
export const getWarehouseStock = async ({ companyId, warehouseId, financialYearId }) => {
  return Stock.find({ companyId, warehouseId, financialYearId, qty: { $gt: 0 } })
    .populate("itemId",      "name code uomId")
    .populate("warehouseId", "name code")
    .sort({ updatedAt: -1 })
    .lean();
